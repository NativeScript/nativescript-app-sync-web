  ...
  return (dispatch) => {
    ...
    .then(data => {
      if (_.get(data,'status') !== "OK") {
        dispatch(addShowMsg("Failed to delete key: "+ _.get(data, 'errorMessage'), "danger"));
      } else {
        dispatch(addShowMsg("Deleted key: " + name, "success"));
      }
...
export function patchAccessKey(name, friendlyName=null, ttl=0) {
  return (dispatch) => {
    ...
    .then(data => {
      if (_.get(data,'status') !== "OK") {
        dispatch(addShowMsg("Changing key failed: "+ _.get(data, 'errorMessage'), "danger"));
      } else {
        dispatch(addShowMsg("Key modified: " + friendlyName, "success"));
      }
...
export function createAccessKey() {
  return (dispatch) => {
    ...
    .then(data => {
      if (_.get(data,'status') !== "OK") {
        dispatch(addShowMsg("Fail to create key: "+ _.get(data, 'errorMessage'), "danger"));
      } else {
        dispatch(addShowMsg("Created key: " + _.get(data, 'results.accessKey.friendlyName'), "success"));
      }
